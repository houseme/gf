...
func (r *Registry) Register(ctx context.Context, service *gsvc.Service) error {
	...
	if err = r.ensureName(service.Key(), data, zk.FlagEphemeral); err != nil {
		return err
	}
	return nil
}
...
func (r *Registry) Deregister(ctx context.Context, service *gsvc.Service) error {
	...
	go func() {
		err := r.conn.Delete(service.Key(), -1)
		ch <- err
	}()
	var err error
	select {
...
// Search get services from zookeeper
func (r *Registry) Search(ctx context.Context, in gsvc.SearchInput) ([]*gsvc.Service, error) {
	serviceNamePath := path.Join(r.opts.rootPath, in.Key())
...
func (r *Registry) Watch(ctx context.Context, serviceName string) (gsvc.Watcher, error) {
	...
	set.lock.Unlock()
	ss, _ := set.services.Load().([]*gsvc.Service)
	if len(ss) > 0 {
